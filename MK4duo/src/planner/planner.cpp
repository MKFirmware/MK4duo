 ...
 */
void Planner::buffer_line(ARG_X, ARG_Y, ARG_Z, const float &e, float fr_mm_s, const uint8_t extruder, const uint8_t driver) {
  ...
  if (max_jerk[E_AXIS + extruder] * 0.5 < fabs(current_speed[E_AXIS])) NOMORE(vmax_junction, max_jerk[E_AXIS + extruder] * 0.5);
  ...
  if ((moves_queued > 1) && (previous_nominal_speed > 0.0001)) {
    ...
    if (dse > max_jerk[E_AXIS + extruder]) NOMORE(vmax_junction_factor, max_jerk[E_AXIS + extruder] / dse);
