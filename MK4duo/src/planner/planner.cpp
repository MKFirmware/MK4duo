...
float Planner::previous_speed[NUM_AXIS],
  ...
  long Planner::axis_segment_time_us[2][3] = { {MAX_FREQ_TIME + 1, 0, 0}, {MAX_FREQ_TIME + 1, 0, 0} };
#endif
  ...
  // Slow down when the buffer starts to empty, rather than wait at the corner for a buffer refill
    ...
    if (moves_queued > 1 && moves_queued < (BLOCK_BUFFER_SIZE) / 2) {
      if (segment_time_us < mechanics.min_segment_time_us) {
        ...
        inverse_mm_s = 1000000.0 / (segment_time_us + LROUND(2 * (mechanics.min_segment_time_us - segment_time_us) / moves_queued));
        #if ENABLED(XY_FREQUENCY_LIMIT)
