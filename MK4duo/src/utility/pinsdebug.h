...
#define _PIN_SAY(NAME) { SERIAL_M(STRINGIFY(NAME)); return true; }
...
static bool report_pin_name(int8_t pin) {
  ...
  if (IS_ANALOG(pin)) {
    SERIAL_C('('); SERIAL_C('A');
    SERIAL_V(int(pin - analogInputToDigitalPin(0)));
    SERIAL_C(')'); SERIAL_C(' ');
  }

  #if defined(RXD) && RXD > -1
    if (pin == 0) { SERIAL_M("RXD"); return true; }
  #endif
  #if defined(TXD) && TXD > -1
    if (pin == 1) { SERIAL_M("TXD"); return true; }
  #endif
    ...
    PIN_SAY(HOTEND_0_AUTO_FAN_PIN);
  #endif
  #if PIN_EXISTS(EXTRUDER_1_AUTO_FAN)
    PIN_SAY(HOTEND_1_AUTO_FAN_PIN);
  #endif
  #if PIN_EXISTS(EXTRUDER_2_AUTO_FAN)
    PIN_SAY(HOTEND_2_AUTO_FAN_PIN);
  #endif
  #if PIN_EXISTS(EXTRUDER_3_AUTO_FAN)
    PIN_SAY(HOTEND_3_AUTO_FAN_PIN);
  #endif
