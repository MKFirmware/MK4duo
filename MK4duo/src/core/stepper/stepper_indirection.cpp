...
//
  ...
  void tmc2130_init(TMC2130Stepper &tmc_driver, const uint16_t mA, const uint16_t microsteps, const uint32_t thrs, const float spmm, const bool tmc_stealthchop=false) {
    while(!tmc_driver.stst()); // Wait for motor stand-still
    #if DISABLED(HYBRID_THRESHOLD)
      UNUSED(thrs);
      UNUSED(spmm);
    #endif
    tmc_driver.begin();
    tmc_driver.setCurrent(mA, R_SENSE, HOLD_MULTIPLIER);
    tmc_driver.microsteps(microsteps);
    tmc_driver.interpolate(INTERPOLATE);
    tmc_driver.stealth_amplitude(STEALTH_AMPL);
    tmc_driver.stealth_gradient(STEALTH_GRAD);
    tmc_driver.stealth_autoscale(STEALTH_AUTOSCALE);
    tmc_driver.stealth_freq(STEALTH_FREQ);
    tmc_driver.stealthChop(tmc_stealthchop);
    tmc_driver.sgt(0);
    #if ENABLED(HYBRID_THRESHOLD)
      tmc_driver.stealth_max_speed(12650000UL * microsteps / (256 * thrs * spmm));
    #endif
    #if ENABLED(TMC2130_LINEARITY_CORRECTION_PRESET)
      tmc2130_set_fixed_wave(tmc_driver, wave);
    #endif
  ...
  void tmc2130_init_to_defaults() {
    ...
    void tmc2130_wr_MSLUTSTART(TMC2130Stepper &tmc_driver, uint8_t start_sin, uint8_t start_sin90) {
      const uint32_t val = uint32_t(start_sin) | (uint32_t(start_sin90) << 16);
      SERIAL_EMV(" MSLUTSTART: ", val);
      tmc_driver.lut_mslutstart(val);
    }

    void tmc2130_wr_MSLUTSEL(TMC2130Stepper &tmc_driver, uint8_t x1, uint8_t x2, uint8_t x3, int8_t w0, int8_t w1, int8_t w2, int8_t w3) {
      ...
      tmc_driver.lut_msutsel(val);
    }

    void tmc2130_wr_MSLUT(TMC2130Stepper &tmc_driver, uint8_t i, uint32_t val) {
      SERIAL_MV(" MSLUT", i);
      SERIAL_EMV(" : ", val);
      switch (i) {
        case 0: tmc_driver.lut_mslut0(val); break;
        case 1: tmc_driver.lut_mslut1(val); break;
        case 2: tmc_driver.lut_mslut2(val); break;
        case 3: tmc_driver.lut_mslut3(val); break;
        case 4: tmc_driver.lut_mslut4(val); break;
        case 5: tmc_driver.lut_mslut5(val); break;
        case 6: tmc_driver.lut_mslut6(val); break;
        case 7: tmc_driver.lut_mslut7(val); break;
        default: break;
      }
    }

    void tmc2130_reset_wave(TMC2130Stepper &tmc_driver) { // TMC2130_LINEARITY_CORRECTION
      SERIAL_EM(" MSLUT RESET ");
      tmc_driver.lut_mslutstart(0x00F70000UL);      // 0x00F70000 16187392
      tmc_driver.lut_mslut0(0xAAAAB554UL);          // 0xAAAAB554 2863314260
      tmc_driver.lut_mslut1(0x4A9554AAUL);          // 0x4A9554AA 1251300522
      tmc_driver.lut_mslut2(0x24492929UL);          // 0x24492929 608774441
      tmc_driver.lut_mslut3(0x10104222UL);          // 0x10104222 269500962
      tmc_driver.lut_mslut4(0xFBFFFFFFUL);          // 0xFBFFFFFF 4227858431
      tmc_driver.lut_mslut5(0xB5BB777DUL);          // 0xB5BB777D 3048961917
      tmc_driver.lut_mslut6(0x49295556UL);          // 0x49295556 1227445590
      tmc_driver.lut_mslut7(0x00404222UL);          // 0x00404222 4211234
      tmc_driver.lut_msutsel(0xFFFF8056UL);         // 0xFFFF8056 4294934614
    }

    void tmc2130_set_fixed_wave(TMC2130Stepper &tmc_driver, const uint8_t i) { // TMC2130_LINEARITY_CORRECTION
      SERIAL_MSG(" MSLUT Wave: ");
      switch (i) {
        case 1:
          SERIAL_EM("SoundCorrection [X]");
          tmc_driver.lut_mslutstart(0x00FC0000UL);
          tmc_driver.lut_mslut0(0xAAAAB554UL);
          tmc_driver.lut_mslut1(0x52AAAAAAUL);
          tmc_driver.lut_mslut2(0x91252529UL);
          tmc_driver.lut_mslut3(0x10208848UL);
          tmc_driver.lut_mslut4(0xBFFC0200UL);
          tmc_driver.lut_mslut5(0xB6DBBBDFUL);
          tmc_driver.lut_mslut6(0x24A55556UL);
          tmc_driver.lut_mslut7(0x00041089UL);
          tmc_driver.lut_msutsel(0xFFFF9156UL);
          break;
        case 2:
          SERIAL_EM("LaserCorrection [X]");
          tmc_driver.lut_mslutstart(0x00FC0000UL);
          tmc_driver.lut_mslut0(0xD52ADB54UL);
          tmc_driver.lut_mslut1(0x52AAAAAAUL);
          tmc_driver.lut_mslut2(0x91252529UL);
          tmc_driver.lut_mslut3(0x10208848UL);
          tmc_driver.lut_mslut4(0xBFFC0200UL);
          tmc_driver.lut_mslut5(0xB6DBBBDFUL);
          tmc_driver.lut_mslut6(0x24A55556UL);
          tmc_driver.lut_mslut7(0x00040089UL);
          tmc_driver.lut_msutsel(0xFFFF9156UL);
          break;
        case 3:
          SERIAL_EM("SoundCorrection [E]");
          tmc_driver.lut_mslutstart(0x00FC0000UL);
          tmc_driver.lut_mslut0(0x6B6DBBDEUL);
          tmc_driver.lut_mslut1(0x4AAAAAADUL);
          tmc_driver.lut_mslut2(0x444924A5UL);
          tmc_driver.lut_mslut3(0x20041044UL);
          tmc_driver.lut_mslut4(0xBEFF8000UL);
          tmc_driver.lut_mslut5(0x5ADB76F7UL);
          tmc_driver.lut_mslut6(0x894A5555UL);
          tmc_driver.lut_mslut7(0x00020844UL);
          tmc_driver.lut_msutsel(0xFFFF8E56UL);
          break;
        case 4:
          SERIAL_EM("LaserCorrection [E]");
          tmc_driver.lut_mslutstart(0x00FD0000UL);
          tmc_driver.lut_mslut0(0xD5BAD554UL);
          tmc_driver.lut_mslut1(0x92A4AAABUL);
          tmc_driver.lut_mslut2(0x91356529UL);
          tmc_driver.lut_mslut3(0x10248848UL);
          tmc_driver.lut_mslut4(0xBFDC2200UL);
          tmc_driver.lut_mslut5(0xB6DABBDBUL);
          tmc_driver.lut_mslut6(0xA4A85552UL);
          tmc_driver.lut_mslut7(0x00040089UL);
          tmc_driver.lut_msutsel(0xFFFF9156UL);
          break;
        default:
    ...
    void tmc2130_set_wave(  TMC2130Stepper &tmc_driver, const uint8_t amp, int16_t fac1000,
      ...
      tmc2130_wr_MSLUTSTART(tmc_driver, 0, amp);
      // Set first W-Bit
      if (wavetype == 1)   // Cycle
...
//
  ...
  // Use internal reference voltage for current calculations. This is the default.
  // Following values from Trinamic's spreadsheet with values for a NEMA17 (42BYGHW609)
  void tmc2208_init(TMC2208Stepper &tmc_driver, const uint16_t mA, const uint16_t microsteps, const uint32_t thrs, const float spmm) {
    tmc_driver.pdn_disable(true); // Use UART
    tmc_driver.mstep_reg_select(true); // Select microsteps with UART
    tmc_driver.I_scale_analog(false);
    tmc_driver.rms_current(mA, HOLD_MULTIPLIER, R_SENSE);
    tmc_driver.microsteps(microsteps);
    tmc_driver.blank_time(24);
    tmc_driver.toff(5);
    tmc_driver.intpol(INTERPOLATE);
    tmc_driver.TPOWERDOWN(128); // ~2s until driver lowers to hold current
    tmc_driver.hysteresis_start(3);
    tmc_driver.hysteresis_end(2);
    #if ENABLED(STEALTHCHOP)
      tmc_driver.pwm_lim(12);
      tmc_driver.pwm_reg(8);
      tmc_driver.pwm_autograd(1);
      tmc_driver.pwm_autoscale(1);
      tmc_driver.pwm_freq(1);
      tmc_driver.pwm_grad(14);
      tmc_driver.pwm_ofs(36);
      tmc_driver.en_spreadCycle(false);
      #if ENABLED(HYBRID_THRESHOLD)
        tmc_driver.TPWMTHRS(12650000UL*microsteps/(256*thrs*spmm));
      #else
        UNUSED(thrs);
        UNUSED(spmm);
      #endif
