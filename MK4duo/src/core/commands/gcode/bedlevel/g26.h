 ...
 */
   ...
   *
   *   G26 is a Mesh Validation Tool intended to provide support for the MK4duo Mesh Bed Leveling System.
   *   In order to fully utilize and benefit from the MK4duo Mesh Bed Leveling System an accurate Mesh must
  ...
  inline bool turn_on_heaters() {
    ...
    printer.setAutoreportTemp(true);
        ...
        if (g26_bed_temp > 25) {
          ...
          while (abs(heaters[BED_INDEX].current_temperature - g26_bed_temp) > 3) {
            ...
            #endif
            printer.idle();
            MKSERIAL.flush(); // G26 takes a long time to complete. PronterFace may
    ...
    while (abs(heaters[0].current_temperature - g26_hotend_temp) > 3) {
      ...
      #endif
      printer.idle();
      MKSERIAL.flush(); // G26 takes a long time to complete. PronterFace can
                        // over run the serial character buffer with M105's without this fix.
    }
  ...
  inline void gcode_G26(void) {
    ...
    if (parser.seenval('B')) {
      ...
      if (!WITHIN(g26_bed_temp, 15, 140)) {
        SERIAL_EM("?Specified bed temperature not plausible.");
        return;
      }
    }
    ...
    do {
      ...
      if (location.x_index >= 0 && location.y_index >= 0) {
        ...
        if (bedlevel.g26_debug_flag) {
          SERIAL_MV("   Doing circle at: (xi=", xi);
          SERIAL_MV(", yi=", yi);
          SERIAL_CHR(')');
          SERIAL_EOL();
        }
