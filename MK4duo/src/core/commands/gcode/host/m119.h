...
inline void gcode_M119(void) {
  ...
  #endif
  #if HAS_DOOR
    SERIAL_EMT(MSG_DOOR_SENSOR, ((READ(DOOR_PIN)^endstops.Is_logic(DOOR_OPEN)) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif
  #if HAS_POWER_CHECK
    SERIAL_EMT(MSG_POWER_CHECK_SENSOR, ((READ(POWER_CHECK_PIN)^endstops.Is_logic(POWEER_CHECK)) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif
}
