...
inline void gcode_M119(void) {
  ...
  #endif
  #if HAS_FIL_RUNOUT
    SERIAL_EMT(MSG_FILAMENT_RUNOUT_SENSOR, ((READ(FIL_RUNOUT_PIN)^endstops.isLogic(FIL_RUNOUT)) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif
  #if HAS_DOOR_OPEN
    SERIAL_EMT(MSG_DOOR_SENSOR, ((READ(DOOR_OPEN_PIN)^endstops.isLogic(DOOR_OPEN)) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif
  #if HAS_POWER_CHECK
    SERIAL_EMT(MSG_POWER_CHECK_SENSOR, ((READ(POWER_CHECK_PIN)^endstops.isLogic(POWER_CHECK)) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif
}
