 ...
 */
  ...
  inline void gcode_G7() {
    ...
    if (parser.seenval('$')) {
      ...
      mechanics.destination.y = mechanics.position.y + (laser.raster_mm_per_pulse * laser.raster_aspect_ratio); // Increment Y axis
    }

    if (parser.seenval('@')) {
      laser.raster_direction = parser.value_int();
      #if ENABLED(LASER_RASTER_MANUAL_Y_FEED)
        mechanics.destination.x = mechanics.position.x; // Don't increment X axis
        mechanics.destination.y = mechanics.position.y; // Don't increment Y axis
      #else
        switch (laser.raster_direction) {
          ...
          case 3:
          case 5:
            mechanics.destination.x = mechanics.position.x + (laser.raster_mm_per_pulse * laser.raster_aspect_ratio); // Increment X axis
          break;
        }
      #endif
    }
    ...
    switch (laser.raster_direction) {
      case 0: // Negative X
        mechanics.destination.x = mechanics.position.x - (laser.raster_mm_per_pulse * laser.raster_num_pixels);
      ...
      case 2: // Negative Vertical
        mechanics.destination.y = mechanics.position.y - (laser.raster_mm_per_pulse * laser.raster_num_pixels);
      ...
      case 4: // Negative X Positive Y 45deg
        mechanics.destination.x = mechanics.position.x - ((laser.raster_mm_per_pulse * laser.raster_num_pixels) * 0.707106);
        mechanics.destination.y = mechanics.position.y + ((laser.raster_mm_per_pulse * laser.raster_num_pixels) * 0.707106);
