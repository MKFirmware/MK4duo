...
void Commands::process_next_gcode() {
  ...
  switch(parser.command_letter) {
    ...
    case 'G': {
      ...
      uint8_t found= 0;
      for (G_CODE_TYPE i = 0; i < COUNT(GCode_Table) && !found; ++i) {
        if (gcode_cmd == GCode_Table[i].code) {
          found= 1;
          GCode_Table[i].command(); // Command found, execute
          break; 
        }
      }
      if(!found){
    ...
    case 'M': {
      const int gcode_cmd = parser.codenum;
      uint8_t found= 0;
      for (M_CODE_TYPE i = 0; i < COUNT(MCode_Table) && !found; ++i) {
        if (gcode_cmd == MCode_Table[i].code) {
          found= 1;
          MCode_Table[i].command(); // Command found, execute
          break; 
        }
      }
      if(!found){
    ...
    case 'T': {
      const int gcode_cmd = parser.codenum;
      uint8_t found= 0;
      for (T_CODE_TYPE i = 0; i < COUNT(TCode_Table) && !found; ++i) {
        if (gcode_cmd == TCode_Table[i].code) {
          found= 1;
          TCode_Table[i].command(); // Command found, execute
          break; 
        }
      }
      if(!found){
