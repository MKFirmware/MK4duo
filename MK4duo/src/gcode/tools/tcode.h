...
inline void gcode_T(void) {
  ...
  uint8_t tool_id = parser.codenum;
    ...
    if (printer.mode == PRINTER_MODE_FFF && (IS_SD_PRINTING || printer.print_job_counter.isRunning()) && printer.previous_extruder != tool_id) {
      gcode_M600();
      printer.previous_extruder = tool_id;
    }
    ...
    if (printer.mode == PRINTER_MODE_FFF) printer.tool_change(
      ...
      (tool_id == printer.active_extruder) || parser.boolval('S')
    );
