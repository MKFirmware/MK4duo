...
inline void gcode_G0_G1(
  ...
  if (printer.IsRunning()) {
    printer.get_destination_from_command(); // For X Y Z E F
      ...
      if (tools.autoretract_enabled && !(parser.seen('X') || parser.seen('Y') || parser.seen('Z')) && parser.seen('E')) {
        const float echange = mechanics.destination[E_AXIS] - mechanics.current_position[E_AXIS];
        // Is this move an attempt to retract or recover?
        if ((echange < -(MIN_RETRACT) && !tools.retracted[tools.active_extruder]) || (echange > MIN_RETRACT && tools.retracted[tools.active_extruder])) {
          ...
          printer.retract(!tools.retracted[tools.active_extruder]);
          return;
        }
      }
    #endif // FWRETRACT
