...
#if HAS_LCD_MENU && ENABLED(GAME_INVADERS)
...
invaders_data_t &idat = game_data.invaders;
...
void InvadersGame::game_screen() {
  ...
  if (lcdui.first_page) {
    ...
    int16_t ep = constrain(int16_t(lcdui.encoderPosition), 0, (LCD_PIXEL_WIDTH - (CANNON_W)) / (CANNON_VEL));
  ...
  if (lcdui.button_pressed()) --idat.quit_count; else idat.quit_count = 10;
...
#endif // HAS_LCD_MENU && ENABLED(GAME_INVADERS)
