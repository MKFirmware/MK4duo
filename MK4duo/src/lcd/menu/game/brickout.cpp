...
#if HAS_LCD_MENU && ENABLED(GAME_BRICKOUT)
...
brickout_data_t &bdat = game_data.brickout;
...
void BrickoutGame::game_screen() {
  if (game_frame()) {   // Run logic twice for finer resolution
    ...
    bdat.paddle_x = constrain(int8_t(lcdui.encoderPosition), 0, (LCD_PIXEL_WIDTH - (PADDLE_W)) / (PADDLE_VEL));
    lcdui.encoderPosition = bdat.paddle_x;
    ...
    if (game_state) do {
      ...
      if (!WITHIN(newx, 0, BTOF(LCD_PIXEL_WIDTH - 1))) {        // out in x?
        bdat.ballh = -bdat.ballh; sound.playtone(5, 220);       // bounce x
      }
      if (newy < 0) {                                           // out in y?
        bdat.ballv = -bdat.ballv; sound.playtone(5, 280);       // bounce v
        bdat.hit_dir = 1;
      }
      ...
      if (WITHIN(newy, BTOF(BRICK_TOP), BTOF(BRICK_BOT))) {
        ...
        if (bdat.bricks[row] & mask) {
          ...
          if ((bdat.ballv < 0) == (bdat.hit_dir < 0)) { bdat.ballv = -bdat.ballv; bdat.ballh += fixed_t(random(-16, 16)); sound.playtone(5, 880); }
          else                                        { bdat.ballh = -bdat.ballh; bdat.ballv += fixed_t(random(-16, 16)); sound.playtone(5, 640); }
        }
      }
      // Is the ball moving down and in paddle range?
        ...
        if (WITHIN(diff, 0, PADDLE_W - 1)) {
          ...
          // Reverse Y direction
          bdat.ballv = -bdat.ballv; sound.playtone(3, 880);
          bdat.hit_dir = -1;
...
void BrickoutGame::enter_game() {
  ...
  reset_bricks(0x0000);
  reset_ball();
  lcdui.encoderPosition = paddle_start / (PADDLE_VEL);
}

#endif // HAS_LCD_MENU && ENABLED(GAME_BRICKOUT)
