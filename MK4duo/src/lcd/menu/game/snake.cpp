...
#if HAS_LCD_MENU && ENABLED(GAME_SNAKE)
...
snake_data_t &sdat = game_data.snake;
...
inline bool food_on_line() {
  for (uint8_t n = 0; n < sdat.head_ind; ++n) {
    ...
    if (p.x == q.x) {
      if ((sdat.foodx == p.x - 1 || sdat.foodx == p.x) && WITHIN(sdat.foody, MIN(p.y, q.y), MAX(p.y, q.y)))
        return true;
    }
    else if ((sdat.foody == p.y - 1 || sdat.foody == p.y) && WITHIN(sdat.foodx, MIN(p.x, q.x), MAX(p.x, q.x)))
      return true;
  }
  return false;
}
...
void SnakeGame::game_screen() {
  ...
  if (game_frame()) do {    // Run logic twice for finer resolution
    ...
    if (oldx != x || oldy != y) {
      ...
      const int8_t enc = int8_t(lcdui.encoderPosition), diff = enc - sdat.old_encoder;
      if (diff) {
