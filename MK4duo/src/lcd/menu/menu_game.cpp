...
#if HAS_LCD_MENU && (ENABLED(GAME_BRICKOUT) || ENABLED(GAME_INVADERS) || ENABLED(GAME_SNAKE) || ENABLED(GAME_MAZE))
...
inline void draw_game_over() {
  ...
  void game_screen_brickout() {
    static int8_t slew;
    lcdui.refresh(LCDVIEW_CALL_NO_REDRAW); // Call as often as possible
  ...
  void lcd_goto_brickout() {
    ...
    reset_bricks(0x0000);
    reset_ball();
    lcdui.goto_screen(game_screen_brickout, paddle_start / (PADDLE_VEL));
  ...
  void lcd_goto_invaders() {
    ...
    reset_invaders();
    reset_player();
    lcdui.goto_screen(game_screen_invaders, 0);
    lcdui.defer_status_screen();
  }
  ...
  void game_screen_snake() {
    ...
    if (game_state && slew-- > 0) do {     // Run logic twice for finer resolution
      ...
      if (oldx != x || oldy != y) {
        ...
        const int8_t enc = int8_t(lcdui.encoderPosition), diff = enc - old_encoder;
        if (diff) {
          old_encoder = enc;
          turn_snake(diff > 0);
        }
  ...
  void lcd_goto_snake() {
    ...
    lcdui.goto_screen(game_screen_snake);
    lcdui.defer_status_screen();
  }

#endif // GAME_SNAKE

#if 1 < ENABLED(GAME_BRICKOUT) + ENABLED(GAME_INVADERS) + ENABLED(GAME_SNAKE)
...
#endif // HAS_LCD_MENU && (GAME_BRICKOUT || GAME_INVADERS || GAME_SNAKE || GAME_MAZE)
