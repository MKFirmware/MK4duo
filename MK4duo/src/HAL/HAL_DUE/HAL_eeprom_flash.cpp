...
#include "../../../MK4duo.h"
  ...
  static bool ee_PageWrite(uint16_t page,const void* data) {
    ...
    if ((status & EEFC_ERROR_FLAGS) != 0) {
      ...
      #if ENABLED(EE_EMU_DEBUG)
        SERIAL_LMV(ECHO, "EEPROM Unlock failure for page ", page);
      #endif
      return false;
    }
    ...
    if ((status & EEFC_ERROR_FLAGS) != 0) {
      ...
      #if ENABLED(EE_EMU_DEBUG)
        SERIAL_LMV(ECHO, "EEPROM Write failure for page ", page);
      #endif
      return false;
    }
    ...
    if (memcmp(getFlashStorage(page),data,PageSize)) {
      ...
      #if ENABLED(EE_EMU_DEBUG)
        SERIAL_LMV(ECHO, "EEPROM Verify Write failure for page ", page);
        ...
        SERIAL_LMV(ECHO, "--> Differing bits: ", count);
      #endif
  ...
  static bool ee_PageErase(uint16_t page) {
    ...
    if ((status & EEFC_ERROR_FLAGS) != 0) {
      ...
      #if ENABLED(EE_EMU_DEBUG)
        SERIAL_LMV("EEPROM Erase failure for page ",page);
      #endif
      return false;
    }
    ...
    for (i = 0; i < PageSize >> 2; i++) {
      if (*aligned_src++ != 0xFFFFFFFF) {
        ...
        #if ENABLED(EE_EMU_DEBUG)
          SERIAL_LMV(ECHO, "EEPROM Verify Erase failure for page ", page);
          ee_Dump( page,(uint32_t *) addrflash);
        #endif
        return false;
      }
  ...
  void eeprom_update_block(const void* pos, void* eeprom_address, size_t n) {
    ...
    const uint8_t* src = (const uint8_t*)eeprom_address;
    while (n--) {
...
#endif // ENABLED(EEPROM_SETTINGS) && DISABLED(EEPROM_I2C) && DISABLED(SPI_EEPROM)
