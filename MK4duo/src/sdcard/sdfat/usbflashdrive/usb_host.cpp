...
uint8_t* MAX3421e::bytesWr(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
  cs();
  HAL::spiSend(reg | 0x02);
  while (nbytes--) HAL::spiSend(*data_p++);
  ncs();
  return data_p;
}
...
uint8_t* MAX3421e::bytesRd(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
  cs();
  HAL::spiSend(reg);
  while (nbytes--) *data_p++ = HAL::spiReceive();
  ncs();
  return data_p;
}
...
bool MAX3421e::start() {
  ...
  if (revision == 0x00 || revision == 0xFF) {
    SERIAL_MV("Revision register appears incorrect on MAX3421e initialization, got ", revision);
    return false;
  }

  if (!reset()) {
    SERIAL_EM("OSCOKIRQ hasn't asserted in time");
    return false;
  }
