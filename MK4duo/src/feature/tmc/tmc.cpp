...
#include "../../../MK4duo.h"
  ...
  void tmc_get_pwmthrs(TMC &st, const char name[], const uint16_t spmm) {
    ...
    SERIAL_EV(12650000UL * st.microsteps() / (256 * st.TPWMTHRS() * spmm));
  }
    ...
    uint8_t monitor_tmc_driver(TMC &st, const char axisID, uint8_t otpw_cnt) {
      TMC_driver_data data = get_driver_data(st);
        ...
        if (data.is_error) {
          SERIAL_EOL();
          SERIAL_TXT(axisID);
          SERIAL_TXT(" driver error detected:");
          if (data.is_ot) SERIAL_TXT("\novertemperature");
          if (st.s2ga()) SERIAL_TXT("\nshort to ground (coil A)");
          if (st.s2gb()) SERIAL_TXT("\nshort to ground (coil B)");
          SERIAL_EOL();
      ...
      if (report_tmc_status) {
        ...
        SERIAL_TXT(axisID);
        SERIAL_MV(":", pwm_scale);
        SERIAL_TXT(" |0b"); SERIAL_VAL(get_status_response(st));
        SERIAL_TXT("| ");
        if (data.is_error) SERIAL_TXT('E');
        else if (data.is_ot) SERIAL_TXT('O');
        else if (data.is_otpw) SERIAL_TXT('W');
        else if (otpw_cnt>0) SERIAL_VAL(otpw_cnt);
        else if (st.flag_otpw) SERIAL_TXT('F');
        SERIAL_TXT("\t");
      }
