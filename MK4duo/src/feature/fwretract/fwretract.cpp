...
// private:
#if MAX_EXTRUDER > 1
  bool FWRetract::retracted_swap[MAX_EXTRUDER];   // Which extruders are swap-retracted
#endif
...
bool  FWRetract::autoretract_enabled,             // M209 S - Autoretract switch
      FWRetract::retracted[MAX_EXTRUDER];         // Which extruders are currently retracted
float FWRetract::current_retract[MAX_EXTRUDER],   // Retract value used by planner
...
void FWRetract::retract(const bool retracting
  ...
  if (retracted[toolManager.extruder.active] == retracting) return;

  #if MAX_EXTRUDER > 1
    // Allow G10 S1 only after G11
    if (swapping && retracted_swap[toolManager.extruder.active] == retracting) return;
    // G11 priority to recover the long retract if activated
    if (!retracting) swapping = retracted_swap[toolManager.extruder.active];
  #else
    constexpr bool swapping = false;
  #endif

  feedrate_t  old_feedrate_mm_s = mechanics.feedrate_mm_s;
  const float unscale_e = RECIPROCAL(extruders[toolManager.extruder.active]->e_factor),
  ...
  if (retracting) {
    ...
    current_retract[toolManager.extruder.active] = base_retract * unscale_e;
    ...
    current_retract[toolManager.extruder.active] = 0.0;
  ...
  retracted[toolManager.extruder.active] = retracting; // Active extruder now retracted / recovered

  // If swap retract/recover then update the retracted_swap flag too
  #if MAX_EXTRUDER > 1
    if (swapping) retracted_swap[toolManager.extruder.active] = retracting;
  #endif
