...
void LEDLights::set_color(const LEDColor &incol
    ...
    const uint32_t neocolor = LEDColorWhite() == incol
                            ? neopixel.strip.Color(NEO_WHITE)
                            : neopixel.strip.Color(incol.r, incol.g, incol.b, incol.w);
    static uint16_t nextLed = 0;

    neopixel.strip.setBrightness(incol.i);
    if (!isSequence)
      neopixel.set_color(neocolor);
    else {
      neopixel.strip.setPixelColor(nextLed, neocolor);
      neopixel.strip.show();
      if (++nextLed >= neopixel.strip.numPixels()) nextLed = 0;
      return;
    }
...
uint8_t LEDLights::getBrightness() {
  #if ENABLED(NEOPIXEL_LED)
    return neopixel.strip.getBrightness();
  #else
    return 255;
  #endif
}
...
void LEDLights::set_white() {
  ...
  #endif
  #if ENABLED(NEOPIXEL_LED)
    neopixel.set_color(neopixel.strip.Color(NEO_WHITE));
  #endif
}
