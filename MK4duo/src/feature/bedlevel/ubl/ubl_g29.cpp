    ...
    if (parser.seen('Q')) {
      ...
      if (parser.seen('J')) {
        if (g29_grid_size) {  // if not 0 it is a normal n x n grid being probed
          ...
          float z3, z2, z1 = probe.check_pt(PROBE_PT_1_X, PROBE_PT_1_Y, false, g29_verbose_level);
          if (!isnan(z1)) {
            z2 = probe.check_pt(PROBE_PT_2_X, PROBE_PT_2_Y, false, g29_verbose_level);
            if (!isnan(z2))
              z3 = probe.check_pt(PROBE_PT_3_X, PROBE_PT_3_Y, true, g29_verbose_level);
          }
          ...
          save_ubl_active_state_and_disable();
          z1 -= get_z_correction(PROBE_PT_1_X, PROBE_PT_1_Y);
          z2 -= get_z_correction(PROBE_PT_2_X, PROBE_PT_2_Y);
          z3 -= get_z_correction(PROBE_PT_3_X, PROBE_PT_3_Y);
    ...
    for (int8_t i = 0; i < GRID_MAX_POINTS_X; i++) {
      for (int8_t j = 0; j < GRID_MAX_POINTS_Y; j++) {
        ...
        if ( (type == INVALID && isnan(z_values[i][j]))  // Check to see if this location holds the right thing
          ...
          distance += HYPOT(mechanics.current_position[X_AXIS] - mx, mechanics.current_position[Y_AXIS] - my) * 0.1;
