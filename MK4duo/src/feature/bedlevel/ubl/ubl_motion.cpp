...
#include "../../../../MK4duo.h"
  ...
  void debug_current_and_destination(const char *title) {
    ...
    SERIAL_MSG("    current=( ");
    SERIAL_VAL(mechanics.current_position[X_AXIS], 6);
    SERIAL_MSG(", ");
    SERIAL_VAL(mechanics.current_position[Y_AXIS], 6);
    SERIAL_MSG(", ");
    SERIAL_VAL(mechanics.current_position[Z_AXIS], 6);
    SERIAL_MSG(", ");
    SERIAL_VAL(mechanics.current_position[E_AXIS], 6);
    SERIAL_MSG(" )   mechanics.destination=( ");
  ...
  void unified_bed_leveling::line_to_destination_cartesian(const float &feed_rate, uint8_t extruder) {
    ...
    if (cell_start_xi == cell_dest_xi && cell_start_yi == cell_dest_yi) { // if the whole move is within the same cell,
      ...
      float z0 = cell_dest_yi < GRID_MAX_POINTS_Y - 1 ? (z1 + (z2 - z1) * yratio) * bedlevel.fade_scaling_factor_for_z(end[Z_AXIS]) : 0.0;
    ...
    if (dxi == 0) {       // Check for a vertical line
      ...
      while (current_yi != cell_dest_yi + down_flag) {
        ...
        float z0 = z_correction_for_x_on_horizontal_mesh_line(x, current_xi, current_yi) * bedlevel.fade_scaling_factor_for_z(end[Z_AXIS]);
    ...
    if (dyi == 0) {             // Check for a horizontal line
      ...
      while (current_xi != cell_dest_xi + left_flag) {
        ...
        float z0 = z_correction_for_y_on_vertical_mesh_line(y, current_xi, current_yi) * bedlevel.fade_scaling_factor_for_z(end[Z_AXIS]);
    ...
    bool _O2 unified_bed_leveling::prepare_segmented_line_to(const float ltarget[XYZE], const float &feedrate) {
      ...
      if (!mechanics.position_is_reachable_xy(ltarget[X_AXIS], ltarget[Y_AXIS]))  // fail if moving outside reachable boundary
        return true; // did not move, so mechanics.current_position still accurate

      const float tot_dx = ltarget[X_AXIS] - mechanics.current_position[X_AXIS],
                  tot_dy = ltarget[Y_AXIS] - mechanics.current_position[Y_AXIS],
                  tot_dz = ltarget[Z_AXIS] - mechanics.current_position[Z_AXIS],
                  tot_de = ltarget[E_AXIS] - mechanics.current_position[E_AXIS];
      ...
      uint16_t    segments = lroundf(mechanics.delta_segments_per_second * seconds),        // preferred number of segments for distance @ feedrate
      ...
      float seg_rx = RAW_X_POSITION(mechanics.current_position[X_AXIS]),
            seg_ry = RAW_Y_POSITION(mechanics.current_position[Y_AXIS]),
            seg_rz = RAW_Z_POSITION(mechanics.current_position[Z_AXIS]),
            seg_le = mechanics.current_position[E_AXIS];

      // Only compute leveling per segment if ubl active and target below z_fade_height.

      if (!bedlevel.leveling_active || !bedlevel.leveling_active_at_z(ltarget[Z_AXIS])) {   // no mesh leveling
        ...
        do {
          ...
          if (--segments) {     // not the last segment
            ...
            seg_rz += seg_dz;
            seg_le += seg_de;
          } else {              // last segment, use exact mechanics.destination
          ...
          mechanics.Transform_segment_raw(seg_rx, seg_ry, seg_rz, seg_le, feedrate);

        } while (segments);

        return false; // moved but did not mechanics.set_current_to_destination();
      }

      // Otherwise perform per-segment leveling

      #if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
        const float fade_scaling_factor = bedlevel.fade_scaling_factor_for_z(ltarget[Z_AXIS]);
      #endif

      // increment to first segment mechanics.destination
      seg_rx += seg_dx;
      seg_ry += seg_dy;
      seg_rz += seg_dz;
      seg_le += seg_de;
      ...
      for(;;) {  // for each mesh cell encountered during the move
        ...
        for(;;) {  // for all segments within this mesh cell
          ...
          mechanics.Transform_segment_raw(seg_rx, seg_ry, seg_rz + z_cxcy, seg_le, feedrate);

          if (segments == 0 )                       // done with last segment
            return false;                           // did not mechanics.set_current_to_destination()
