...
void unified_bed_leveling::G26_line_to_destination(const float &feed_rate) {
  ...
  bool user_canceled() {
    ...
    printer.safe_delay(10);                       // Wait for click to settle
    ...
    while (!ubl_lcd_clicked()) printer.idle();    // Wait for button release

    // If the button is suddenly pressed again,
    // ask the user to resolve the issue
    lcd_setstatusPGM(PSTR("Release button"), 99); // will never appear...
    while (ubl_lcd_clicked()) printer.idle();             // unless this loop happens
    lcd_reset_status();
...
bool unified_bed_leveling::prime_nozzle() {
    ...
    if (g26_prime_flag == -1) {  // The user wants to control how much filament gets purged
      ...
      while (ubl_lcd_clicked()) printer.idle(); // Debounce Encoder Wheel
