...
class unified_bed_leveling {
  ...
  public:
    ...
    static float  _mesh_index_to_xpos[GRID_MAX_POINTS_X],
    ...
    static float get_z_correction(const float &rx0, const float &ry0) {
      ...
      if (printer.debugMesh()) {
        DEBUG_MV(" raw get_z_correction(", rx0);
        DEBUG_CHR(',');
        DEBUG_VAL(ry0);
        DEBUG_MV(") = ", z0, 6);
        DEBUG_EMV(" >>>---> ", z0, 6);
      ...
      if (isnan(z0)) { // if part of the Mesh is undefined, it will show up as NAN
        ...
        if (printer.debugMesh()) {
          DEBUG_MV("??? Yikes!  NAN in get_z_correction(", rx0);
          DEBUG_CHR(',');
          DEBUG_VAL(ry0);
          DEBUG_CHR(')');
          DEBUG_EOL();
    ...
    static inline float mesh_index_to_xpos(const uint8_t i) {
      return i < GRID_MAX_POINTS_X ? _mesh_index_to_xpos[i] : MESH_MIN_X + i * (MESH_X_DIST);
    }
    static inline float mesh_index_to_ypos(const uint8_t i) {
      return i < GRID_MAX_POINTS_Y ? _mesh_index_to_ypos[i] : MESH_MIN_Y + i * (MESH_Y_DIST);
    }
