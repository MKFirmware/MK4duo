  ...
  public:
    ...
    inline static float z_correction_for_x_on_horizontal_mesh_line(const float &lx0, const int x1_i, const int yi) {
      if (!WITHIN(x1_i, 0, GRID_MAX_POINTS_X - 2) || !WITHIN(yi, 0, GRID_MAX_POINTS_Y - 1)) {
          ...
          if (DEBUGGING(LEVELING)) {
            SERIAL_PS( !WITHIN(x1_i, 0, GRID_MAX_POINTS_X - 1) ? PSTR("x1l_i") : PSTR("yi") );
            SERIAL_MV(" out of bounds in z_correction_for_x_on_horizontal_mesh_line(lx0=", lx0);
            SERIAL_MV(",x1_i=", x1_i);
            SERIAL_MV(",yi=", yi);
            SERIAL_CHR(')');
            SERIAL_EOL();
    ...
    inline static float z_correction_for_y_on_vertical_mesh_line(const float &ly0, const int xi, const int y1_i) {
      if (!WITHIN(xi, 0, GRID_MAX_POINTS_X - 1) || !WITHIN(y1_i, 0, GRID_MAX_POINTS_Y - 2)) {
        #if ENABLED(DEBUG_LEVELING_FEATURE)
          if (DEBUGGING(LEVELING)) {
            SERIAL_PS( !WITHIN(xi, 0, GRID_MAX_POINTS_X - 1) ? PSTR("xi") : PSTR("yl_i") );
            SERIAL_MV(" out of bounds in z_correction_for_y_on_vertical_mesh_line(ly0=", ly0);
            SERIAL_MV(", xi=", xi);
            SERIAL_MV(", y1_i=", y1_i);
            SERIAL_CHR(')');
            SERIAL_EOL();
    ...
    static float get_z_correction(const float &lx0, const float &ly0) {
      ...
      if (!WITHIN(cx, 0, GRID_MAX_POINTS_X - 2) || !WITHIN(cy, 0, GRID_MAX_POINTS_Y - 2)) {
        ...
        SERIAL_MV("? in get_z_correction(lx0=", lx0);
        SERIAL_MV(", ly0=", ly0);
        SERIAL_CHR(')');
        SERIAL_EOL();
      ...
      float z0 = calc_z0(RAW_Y_POSITION(ly0),
        ...
        if (DEBUGGING(MESH_ADJUST)) {
          SERIAL_MV(" raw get_z_correction(", lx0);
          SERIAL_CHR(',');
          SERIAL_VAL(ly0);
          SERIAL_MV(") = ", z0, 6);
        }
      ...
      if (isnan(z0)) { // if part of the Mesh is undefined, it will show up as NAN
        ...
        #if ENABLED(DEBUG_LEVELING_FEATURE)
          if (DEBUGGING(MESH_ADJUST)) {
            SERIAL_MV("??? Yikes!  NAN in get_z_correction(", lx0);
            SERIAL_CHR(',');
            SERIAL_VAL(ly0);
            SERIAL_CHR(')');
            SERIAL_EOL();
     ...
     *
     *  Returns 1.0 if bedlevel.z_fade_height is 0.0.
     *  Returns 0.0 if Z is past the specified 'Fade Height'.
     */
    #if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
      static inline float fade_scaling_factor_for_z(const float &lz) {
        if (bedlevel.z_fade_height == 0.0) return 1.0;
        static float fade_scaling_factor = 1.0;
        const float rz = RAW_Z_POSITION(lz);
        if (last_specified_z != rz) {
          last_specified_z = rz;
          fade_scaling_factor =
            rz < bedlevel.z_fade_height
              ? 1.0 - (rz * bedlevel.inverse_z_fade_height)
              : 0.0;
        }
